...
class ResBlock(nn.Module):
    def __init__(self, nio, nhid, dropout=0.1):
        super(ResBlock, self).__init__()

        self.res = nn.Sequential(nn.Linear(nio, nhid), nn.LayerNorm(nhid), nn.ReLU(),
                                 nn.Linear(nhid, nio), nn.LayerNorm(nio),
                                 nn.Dropout(dropout))
        self.act = nn.ReLU()
...
class ResNet(BaseNet):
    def __init__(self, depth, width, multitask=True):
        super(ResNet, self).__init__(width, multitask=multitask)
        assert(width % 64 == 0)
        nhead, ndense = width//64, width*4
        ...
        self.encod = nn.Sequential(*[ResBlock(width, ndense) for i in range(depth)])
...
class ResTransNet(BaseNet):
    def __init__(self, dres, dtrans, width, multitask=True):
        super(ResTransNet, self).__init__(width, multitask=multitask)
        assert(width % 64 == 0)
        nhead, ndense = width//64, width*4
        ...
        self.encod_res = nn.Sequential(*[ResBlock(width, ndense) for i in range(dres)])
        layer_encod_trans = nn.TransformerEncoderLayer(width, nhead, dim_feedforward=ndense, dropout=0.1)
        self.encod_trans = nn.TransformerEncoder(layer_encod_trans, dtrans)

    def forward(self, x, mask):
        mem = self.encod_res(self.embed(x)).masked_fill_(mask.unsqueeze(2), 0)
        mem = self.encod_trans(mem.permute(1, 0, 2), src_key_padding_mask=mask).permute(1, 0, 2).masked_fill_(mask.unsqueeze(2), 0)
        mem = mem.sum(1) / (mem.size(1) - mask.float().unsqueeze(2).sum(1))
        return super().forward(mem)
